...
for i, filename in enumerate(args.filenames):
    ...
    for wave_id in wave_ids:
        ...
        avg_y = np.average(directionY)
        avg_x = np.average(directionX)
...
avg_df = pd.DataFrame(lst, columns=['avg_x', 'avg_y'], index = wave_ids)

# Calculate angles using arctan2
angles = np.arctan2(all_directionY, all_directionX)
angles_w_avg = np.arctan2(avg_df['avg_y'], avg_df['avg_x'])
magnitudes = np.linalg.norm(np.vstack([all_directionX, all_directionY]), axis=0)
magnitudes1 = np.linalg.norm(np.vstack([avg_df['avg_x'], avg_df['avg_y']]), axis=0)

# Calculate the weighted average angle
weighted_average_angle = np.arctan2(np.average(all_directionY, weights=magnitudes, axis=0), 
                                    np.average(all_directionX, weights=magnitudes, axis=0))
weighted_average_angle1 = np.arctan2(np.average(avg_df['avg_y'], weights=magnitudes1, axis=0), 
                                    np.average(avg_df['avg_x'], weights=magnitudes1, axis=0))
...
ax.hist(angles, bins=36, range=(-np.pi, np.pi), density=True, weights=magnitudes)

fig1, ax1 = plt.subplots(subplot_kw={'projection': 'polar'})
ax1.hist(angles_w_avg, bins=36, range=(-np.pi, np.pi), density=True, weights=magnitudes1)
print(angles_w_avg)

# Plot the weighted average line
ax.plot([0, weighted_average_angle], [0, ax.get_ylim()[1]], color='red', linewidth=2)
ax1.plot([0, weighted_average_angle1], [0, ax1.get_ylim()[1]], color='blue', linewidth=2)
